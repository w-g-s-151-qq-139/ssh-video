...
<body class="w100">
			...
			<div class="proflle_tab_body">
				<div class="proflle_tab_workplace clearfix">
					...
					<div class="profile_ifo_area">
						<form
							action="${pageContext.request.contextPath }/user/keepprofile.action"
							method="post">
							<div class="form_group">
								<span class="dd">昵&#x3000;称：</span><input type="text"
									name="nickName" value="${user.nickName}">
							</div>
							<div class="form_group">
								<span class="dd">性&#x3000;别：</span> <input type="radio" id="man"
									<c:if test="${user.sex==1}">checked='checked'</c:if> value="1"
									name="sex"><label for="man">男</label> <input
									type="radio" id="woman"
									<c:if test="${user.sex==2}">checked='checked'</c:if> value="2"
									name="sex"><label for="woman">女</label>
							</div>
							<div class="form_group">
								<span class="dd">生&#x3000;日：</span> <input type="date"
									name="birthday1"
									value="<fmt:formatDate pattern='yyyy-MM-dd' value='${user.birthday }'/>">
							</div>
							<div class="form_group">
								<span class="dd">邮&#x3000;箱：</span> <input type="text"
									style="border: none" name="email" readonly="readonly"
									value="${user.email}">
							</div>
							<div class="form_group">
								<span class="dd">所在地：</span> <input type="hidden" id="uprovince"
									value="${user.province }"> <input type="hidden"
									id="ucity" value="${user.city }">
								<div id="city">
									<select class="prov" name="province"></select> <select
										class="city" name="city"></select>
								</div>
							</div>
							<div class="form_submit dd">
								<input type="submit" value="保&#x3000;存"> <a
									href="${pageContext.request.contextPath }/user/profile.action">重置</a>
							</div>
						</form>
					</div>
				</div>
	...
	<script
		src="${pageContext.request.contextPath }/js/jquery.cityselect.min.js"></script>
